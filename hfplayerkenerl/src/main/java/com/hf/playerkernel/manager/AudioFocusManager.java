...
public class AudioFocusManager implements AudioManager.OnAudioFocusChangeListener {
    ...
    private boolean willPlay() {
        ...
        return mPlayService.playback.isPreparing() || mPlayService.playback.isPlaying();
    }


    private void forceStop() {
        //当准备播放时，则停止播放
        if (mPlayService.playback.isPreparing()) {
            mPlayService.playback.stop();
            //当正在播放时，则暂停播放
        } else if (mPlayService.playback.isPlaying()) {
            mPlayService.playback.pause();
        }
    }
